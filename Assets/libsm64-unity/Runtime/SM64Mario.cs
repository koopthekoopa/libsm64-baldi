...
namespace LibSM64
{
    public class SM64Mario : MonoBehaviour
    {
        ...
        public void contextFixedUpdate()
        {
            ...
            if (states[buffIndex].hasMetalCap)
            {
        ...
        public void contextUpdate()
        {
            ...
            marioRotation = states[buffIndex].unityRotation;
        ...
        public void SetAction( SM64ActionType action )
        {
            Interop.SetMarioAction(marioId, action);
        }

        public void ResetSpeed()
        {
            Interop.SetMarioVelocity(marioId, Vector3.zero);
            Interop.SetMarioForwardVelocity(marioId, 0);
        }

        public void SetForwardVelocity(float velocity)
        {
            Interop.SetMarioForwardVelocity(marioId, velocity);
        }

        public void TeleportTo( Vector3 position )
        {
            Interop.SetMarioPosition(marioId, position);
            ResetSpeed();
        }

        public void GiveWingCap( ushort capTime, bool playMusic = true )
        {
            Interop.InteractCap( marioId, SM64CapType.Wing, capTime, playMusic );
        }

        public void GiveMetalCap( ushort capTime, bool playMusic = true )
        {
            Interop.InteractCap( marioId, SM64CapType.Metal, capTime, playMusic );
        }

        public bool HasMetalCap()
        {
            return states[buffIndex].hasMetalCap;
        }

        public bool IsLongJumping()
        {
            return states[buffIndex].isLongJumping;
        }
